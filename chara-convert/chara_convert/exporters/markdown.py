...
# Display names + copy-paste hints for layered targets (FictionLab so far).
...
_LAYER_HINTS: dict[str, str] = {
    "character": "Paste into FictionLab → Characters → Create New.",
    "location": "Paste into FictionLab → Locations → Create New.",
    "scenario": "Paste into FictionLab → Scenarios → Create New. "
    "First Message lives here, NOT in the Character Card.",
    "lore": "Paste into FictionLab → Lore Pieces → Create New. "
    "Each Lore Piece typically gets its own entry.",
}
...
def render_markdown(converted: ConvertedCard, gap: GapReport) -> str:
    ...
    # Layered target (FictionLab) — render one section per layer.
    if converted.layers is not None:
        for lname, fields in converted.layers.items():
            label = _LAYER_LABELS.get(lname, lname.title())
            lines.append(f"## {label}")
            lines.append("")
            hint = _LAYER_HINTS.get(lname)
            if hint:
                lines.append(f"> {hint}")
                lines.append("")
            if not fields:
                lines.append(_MANUAL_PLACEHOLDER)
                lines.append("")
                continue
            for fname, value in fields.items():
                lines.append(f"### {fname.replace('_', ' ').title()}")
                lines.append("")
                lines.append(value)
                lines.append("")
        lines.append("---")
        lines.append("")
        lines.append(
            f"*Generated by chara-convert for {gap.target_name}*"
        )
        return "\n".join(lines)
