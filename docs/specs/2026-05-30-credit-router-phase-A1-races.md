...
- T19 — pending. Adversarial review pass focusing on: DO eviction mid-batch, D1 partial-failure inside DO handler, cron+live-traffic interleaving, error mapping (DO 5xx propagation), `blockConcurrencyWhile` deadlock corners.
...
- **Cron sweep contention** — if cron tries to refund 500 stale holds and live traffic is heavy, the DO queue per user may build up. Cron LIMIT 500 + per-row DO call keeps it bounded; if measured contention is a problem, lower LIMIT or add jitter.
