...
- ~~**A.1.x-M1** (T19)~~ — **DONE 2026-05-29** — CreditError union extended (`service_unavailable` / `internal_error` / `not_found`). `index.ts` DO-unreachable → `503 service_unavailable`. `credit-do.ts` unexpected throws in hold/debit/refund → `500 internal_error`; unknown op → `404 not_found`. Worker catch does not currently distinguish D1-transient from true internals (both → `500 internal_error`); D1 retry happens inside the DO via D1's own client. Phase B `credit_client.py` matches with: 5xx → `BillingServiceUnavailable`; idempotent ops (balance/debit/refund) retry on 5xx with 50→100ms backoff; hold never retries (non-idempotent — orphan-hold cron sweeps any phantom). Tests: `workers/billing/test/endpoints.test.ts` (3 new) + `chara-convert/tests/llm/test_credit_client.py` (9 new).
...
- **A.1.x-m2/m3** (T19, polish) — `index.ts:39-40` cron comment says "404 = hold deleted" but `refund()` returns idempotent 200; update wording. (~~`credit-do.ts:93` returns 404 for unknown op; should be 400 per HTTP semantics~~ — kept as 404 with `not_found` code as part of M1; the route truly doesn't exist for that DO endpoint, 404 is correct.)
